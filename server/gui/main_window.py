...
from gui.widgets.loading_widget import LoadingOverlay
from gui.themes import get_dark_theme_stylesheet
...
class MainWindow(QMainWindow, ServerControlMixin, ConfigurationMixin):
    ...
    def setup_ui(self):
        ...
        # Overlay de chargement (masqué par défaut)
        self.loading_overlay = LoadingOverlay(parent=central_widget)
    ...
    def start_job_async(self, file_path):
        ...
        try:
            ...
            self.loading_overlay.set_message("Création du job...")
            self.loading_overlay.set_detail(f"Analyse de {Path(file_path).name}")
            ...
            try:
                ...
                if job:
                    ...
                    self.loading_overlay.set_message("Extraction des frames...")
                    self.loading_overlay.set_detail(f"Traitement en cours...")
                    ...
                    if success:
                        ...
                        estimated_time = self._estimate_job_completion_time(job)
                        ...
                        QMessageBox.information(self, "Succès",
                            ...
                            f"Pistes audio: {len(job.audio_tracks)}\n"
                            f"Sous-titres: {len(job.subtitle_tracks)}\n"
    ...
    def _estimate_job_completion_time(self, job) -> str:
        """Estime le temps de complétion d'un job"""
        # Estimation basique: ~2 secondes par frame (peut être ajusté selon les performances)
        seconds_per_frame = 2
        total_seconds = job.total_frames * seconds_per_frame
