"""
Onglet Mise à jour - Configuration et gestion des mises à jour
...
class UpdateTab(QWidget):
    """Onglet de gestion des mises à jour"""
    ...
    def LoadConfiguration(self):
        ...
        try:
            ...
            if Database:
                # Canal
                Channel = Database.GetParameter("update_channel", UpdateConfig.DEFAULT_CHANNEL)
                Index = self.ChannelCombo.findData(Channel)
                if Index >= 0:
                    self.ChannelCombo.setCurrentIndex(Index)

                # Auto-check
                AutoCheck = Database.GetParameterBool("update_auto_check", False)
    ...
    def OnChannelChanged(self, Index: int):
        ...
        Database = self.ParentWindow.GetDatabase()
        if Database:
            Database.SetParameter("update_channel", Channel, "Canal de mise à jour")
